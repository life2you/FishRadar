...
- `docker-compose.yaml` 默认会启动 MySQL 8 作为主库，应用通过 `APP_DATABASE_URL` 连接
- 目前默认持久化这些目录：
    - `mysql_data`  MySQL 主存储（任务、结果、价格历史）
    - `state/`  登录状态 cookie 文件
    - `prompts/`  任务提示词
    - `logs/`  运行日志
    - `images/`  商品图片与任务临时图片目录
    - `config.json`、`jsonl/`、`price_history/`  首次迁移时用于兼容导入的旧数据源
...
- 首次迁移完成并确认数据库中数据正确后，可视部署方式决定是否继续保留旧的 `config.json`、`jsonl/`、`price_history/` 挂载
...
- 结果页和导出功能现在从数据库查询，不再直接扫描 `jsonl` 文件。
